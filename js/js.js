...
{
  cityName: "Amsterdam",
  cityInfo: "The European languages are members of the same family. Their separate existence is a myth. For science, music, sport, etc, Europe uses the same vocabulary. The languages only differ in their grammar, their pronunciation and their most common words. Everyone realizes why a new common language would be desirable: one could refuse to pay expensive translators.",
...
var galleryItmsToClick = document.getElementById("destinationsGallery").getElementsByTagName("figure");
var galleryItmsToClickLength = galleryItmsToClick.length;
var closeModalXBttn = document.getElementById("closeModalXBttn").addEventListener("click", toggleModalVisibility);
for(var i=0; i<galleryItmsToClickLength; i++){
  galleryItmsToClick[i].addEventListener("click", createModalWindow);
}


function fillModalWindow(arg){
  var chosenCityName = arg.children[2].textContent;
  document.getElementById("cityName").textContent = chosenCityName;
  var searchedCity = citiesInfo.find(function (arg) {
    return (arg.cityName == chosenCityName);
});
  document.getElementById('cityInfo').textContent = searchedCity.cityInfo;
  document.getElementById('cityPicture').src = searchedCity.cityPicture;
}
